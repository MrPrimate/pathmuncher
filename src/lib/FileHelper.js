...
const FileHelper = {
  ...
  fileExistsUpdate: (fileList) => {
    const targetFiles = fileList.filter((f) => !CONFIG.PATHMUNCHER.KNOWN.FILES.has(f));
    for (const file of targetFiles) {
      CONFIG.PATHMUNCHER.KNOWN.FILES.add(file);
    }
  },

  generateCurrentFiles: async (directoryPath) => {
    if (!CONFIG.PATHMUNCHER.KNOWN.CHECKED_DIRS.has(directoryPath)) {
      ...
      if (typeof ForgeVTT !== "undefined" && ForgeVTT?.usingTheForge) {
        if (fileList.bazaar) {
          ...
          CONFIG.PATHMUNCHER.KNOWN.FORGE.TARGETS[directoryPath] = {};
          fileList.files.forEach((file) => {
            const fileName = file.split("/").pop();
            CONFIG.PATHMUNCHER.KNOWN.FORGE.TARGETS[directoryPath][fileName] = file;
            CONFIG.PATHMUNCHER.KNOWN.FILES.add(file);
          });
        } else {
          ...
          // eslint-disable-next-line require-atomic-updates
          CONFIG.PATHMUNCHER.KNOWN.FORGE.TARGET_URL_PREFIX[directoryPath] = `https://assets.forge-vtt.com/${userId}/${dir.current}`;
        }
      }

      CONFIG.PATHMUNCHER.KNOWN.CHECKED_DIRS.add(directoryPath);
    } else {
  ...
  fileExists: async (directoryPath, filename) => {
    ...
    let existingFile = CONFIG.PATHMUNCHER.KNOWN.FILES.has(fileUrl);
    ...
    const filePresent = CONFIG.PATHMUNCHER.KNOWN.FILES.has(fileUrl);
    ...
    const result = new Promise((resolve) => {
      ...
      .then((rawImage) => {
        ...
        return new Promise((resolve) => {
          ...
          const quality = game.settings.get(CONSTANTS.MODULE_ID, "webp-quality");
  ...
  uploadFile: async function (data, path, filename, forceWebp = false) {
    const useWebP = game.settings.get(CONSTANTS.MODULE_ID, "use-webp");
  ...
  uploadRemoteImage: async function (url, targetDirectory, baseFilename, useProxy = false) {
    // prepare filenames
    ...
    try {
      // const urlEncode = game.settings.get(CONSTANTS.MODULE_ID, "cors-encode");
      ...
      CONFIG.PATHMUNCHER.KNOWN.FILES.add(result);
      return result;
    } catch (error) {
  ...
  getForgeUrl: async (directoryPath, filename) => {
    ...
    const prefix = CONFIG.PATHMUNCHER.KNOWN.FORGE.TARGET_URL_PREFIX[directoryPath];
    const bazaarTargetPath = CONFIG.PATHMUNCHER.KNOWN.FORGE.TARGETS[directoryPath];
